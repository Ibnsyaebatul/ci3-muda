...
<body>
<header><h1><?= isset($row)?'Edit':'Tambah' ?> Data Siswa</h1></header>
<div class="container">
  <p><a href="<?= site_url('siswa'); ?>">← Kembali</a></p>

  <div class="card">
    <?php
      // >>>> KUNCI: selalu definisikan $row
      if (!isset($row) || !is_object($row)) { $row = null; }
      $is_edit = ($row !== null);

      // helper aman tanpa use()
      function form_val($key, $is_edit, $row, $default=''){
        return set_value($key, ($is_edit && isset($row->$key)) ? $row->$key : $default);
      }
      $jk = form_val('jenis_kelamin', $is_edit, $row);
    ?>
    ...
    <?= form_open(current_url(), ['autocomplete'=>'off']) ?>
      ...
      <div class="grid">
        ...
        <div>
          <input type="text" id="nis" name="nis" maxlength="15" value="<?= form_val('nis',$is_edit,$row) ?>">
          <?= form_error('nis','<small class="err">','</small>'); ?>
        </div>

        <label for="nama">Nama</label>
        <div>
          <input type="text" id="nama" name="nama" maxlength="30" value="<?= form_val('nama',$is_edit,$row) ?>">
        ...
        <div>
          <input type="text" id="tempat_lahir" name="tempat_lahir" maxlength="50" value="<?= form_val('tempat_lahir',$is_edit,$row) ?>">
          <?= form_error('tempat_lahir','<small class="err">','</small>'); ?>
        </div>

        <label for="tanggal_lahir">Tanggal Lahir</label>
        <div>
          <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="<?= form_val('tanggal_lahir',$is_edit,$row) ?>">
        ...
        <div>
          <input type="text" id="alamat" name="alamat" maxlength="100" value="<?= form_val('alamat',$is_edit,$row) ?>">
